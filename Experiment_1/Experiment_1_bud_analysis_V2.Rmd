...
Late-season temperature is represented by summer cooling treatment the combination of treatment timing (July vs August) and cooling level (Moderate vs Extreme).

``` {r}
######## ICC
BudsSummary = BudsSummary %>% 
  mutate(Treatment.bud.type = paste(Treatment, bud.type, sep = "."))

m <- lmer(EOS.DOY ~ 1 + (1|Treatment.bud.type), data=BudsSummary)

vc <- as.data.frame(VarCorr(m))
v_treat <- vc$vcov[vc$grp=="Treatment.bud.type"]
v_resid <- vc$vcov[vc$grp=="Residual"]
ICC <- v_treat / (v_treat + v_resid)
ICC # output: 0.2587
